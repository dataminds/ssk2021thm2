...
- 설문전문업체에 등록된 패널을 성, 세대, 지역을 층화변수로 할당표집했다. 총 47,589명에게 설문참가 안내 이메일을 보냈다. 8,196명이 설문안내 이메일을 통해 접속했다. 이중 5,967명이 응답을 완료했다. 불성실 응답자를 제외한 5889명(남성 = `r sex_df$N[1]`; 나이 *M* = `r age_df$M`, *SD* = `r age_df$SD`)을 최종 분석 대상에 포함했다.
...
4개 유형(같이 살고 있는 사람, 비동거 가족 혹은 친척, 업무/학업 목적의 지인/친구/동료, 사적인 목적의 지인/친구/동료)의 사람들과의 접촉 빈도("지난 한 달 평균 귀하는 다음의 사람들과 얼마나 자주 직접 만나서 대화를 하셨습니까?")를 통해 측정했다(1 = 전혀 안했다, 9 = 한 시간에 여러번). 
...
df %>% 
  mutate(정보이용상하 = case_when(
    정보이용 <= quantile(df$정보이용, probs = .3) ~ 3,
    정보이용 >= quantile(df$정보이용, probs = .7) ~ 7, 
    TRUE ~ 5)) -> df
...
- 대면소통 - 고립감 - 불안녕감: 매개관계 유지
 
- 대면소통 - 소속연결 - 불안녕감: 정보이용(상)에서 매개관계 소멸
...
df %>% 
  mutate(오락이용상하 = case_when(
    오락이용 <= quantile(df$오락이용, probs = .3) ~ 3,
    오락이용 >= quantile(df$오락이용, probs = .7) ~ 7, 
    TRUE ~ 5)) -> df
...
---
name: post1a
# 사후분석1: 정보/오락이용-안념감 매개변수 탐색

정보이용과 오락이용의 증가가 안녕감 증가에 유의하게 기여하지만, 소속연결감의 매개효과는 부분적. 다른 매개변수 존재 여부를 판단하기 위해서 안녕감에 대해 모든 연구변수를 독립변수로 투입하고 회귀분석한 결과, 대면소통과 관계이용의 유의성은 남아 있고, 정보이용과 오락이용의 유의성만 사라졌다. 
...
---
name: post2a
# 사후분석2: 관계이용의 정보/오락이용-안념감 매개

.left-column3[

정보이용과 오락이용의 안녕감의 관계에 대해 대면소통과 관계이용을 번갈아가며 매개변수로 투입해 총 4회의 매개분석을 한 결과, 대면빈도는 매개하지 않았고, 관계이용이 매개. 
]
...
for(i in seq_along(M)) {
  for (j in seq_along(X)) {
    M_fmla <- paste0(M[[i]], "~", X[[j]], "+", W)
    Y_fmla <- paste0(Y, "~", X[[j]], "+", M[[i]], "+", W)
    ...
    plot(med_out, main = paste(X[[j]], "-", M[[i]], "-", Y, sep = " "))
    out_l[[i]] <- summary(med_out)
  }
}
...
---
name: post3a
# 사후분석3: 관계이용-안념감 소속연결감 매개에 정보/오락이용 통제

.left-column3[

정보/오락이용을 통제하고 관계이용의 안녕감 관계에 대해 소속연결감을 매개변수로 투입. 차이 없음.
]

.right-column7[

```{r}
X <- c("관계이용", "정보이용", "오락이용")[1]
M <- c("관계_소속연결")
Y <- "삶만족"
W <- "성+나이+소득+학력+정보이용+오락이용"
...
---
name: cluster1
# 군집분석1: 관계-정보이용 군집해 관계이용-안념감 소속연결감 매개
...
# 군집
v5889_df %>%
  dplyr::select(관계이용, 정보이용) -> df;df %>% 
  kmeans(centers = n) -> kclust
# 군집한 집단 시각화
kclust %>% tidy() %>% 
  pivot_longer(c(관계이용,  정보이용), 
                 names_to = "이용", values_to = "이용값")  %>% 
  ggplot(aes(x = cluster, y =  이용값, fill =  이용)) +
  geom_bar(position = "dodge", stat = "identity")

```


HL군집은 소속연결이 관계이용-삶만족 완전매개
LL군집은 소속연결이 관계이용-삶만족 부분매개
HH군집은 소속연결이 관계이용-삶만족 부분매개
MM군집은 매개 없음. total effect 소멸. 직접효과는 삶만족 악화.
...
```{r}
# 군집데이터프레임 + 원데이터셋
inner_join(v5889_df, augment(kclust, df)) %>% 
  rename(군집 = .cluster) %>% 
  mutate(군집 = recode(
    군집, `1` = "HL", `2` = "LL",  `3` = "HH", `4` = "MM")) -> c_df

op <- par(no.readonly = TRUE)
par(mfrow = c(2,2))

for(k in c("HL", "LL", "HH", "MM")) {
c_df %>% filter(군집 == k) -> df

X <- c("대면빈도", "관계이용", "정보이용", "오락이용")[c(2)]
M <- c("관계_고립", "관계_소속연결")[2]
Y <- c("우울불안", "삶만족")[2]
W <- "성+나이+소득+학력"
...
---
name: cluster2
# 군집분석2: 관계-오락이용 군집해 관계이용-안념감 소속연결감 매개
...
# 군집
v5889_df %>%
  dplyr::select(관계이용, 오락이용) -> df;df %>% 
  kmeans(centers = n) -> kclust
# 군집한 집단 시각화
kclust %>% tidy() %>% 
  pivot_longer(c(관계이용,  오락이용), 
                 names_to = "이용", values_to = "이용값")  %>% 
  ggplot(aes(x = cluster, y =  이용값, fill =  이용)) +
  geom_bar(position = "dodge", stat = "identity")
...
HL군집은 소속연결이 관계이용-삶만족 완전매개
LL군집은 소속연결이 관계이용-삶만족 부분매개
HH군집은 소속연결이 관계이용-삶만족 완전매개
MM군집은 매개 없음. total effect = ADE.
...
모든 유형의 미디어이용과 대면소통 시간 사이에 정의 상관성(스마트기기는 상관이 유의하지 않다): 전치가설 배제 가능
...
- 관계/정보/오락 이용: 
  ...
  - 불안녕감: 고립감과 소속연결감 불안녕감 감소를 매개하지 않았다.
  - 관계이용은 불안녕감과 관련이 없지만, 정보이용과 오락이용은 불안녕감 증대.   - 매개변수: 고립감은 안녕감을 매개하지 않았고, 소속연결감이 안녕감을 부분적으로 매개.  
  
- 정보이용과 오락이용은 불안녕감과 안녕감을 모두 증대하는 효과.
...
- 관계이용, 정보이용, 오락이용 모두 대면소통의 불안녕감과 안녕감의 관계를 부정적으로 조절. 즉, 관계이용, 정보이용, 오락이용 시간이 많은 집단에서 대면소통의 안녕감과 불안녕감에 대한 긍정효과 소멸
...
- 정보이용과 오락이용은 불안녕감 증가에 기여. 

  - 대면소통을 전치하기 때문은 아니다. 
  - 정보이용과 오락이용 자체의 불안녕감 증가 요인 가능 (보상민감성, 충동성 강화 등).
  - 관계이용은 불안녕감 증가에 기여하지 않는다. 

- 세 유형의 미디어이용 모두 안녕감 증가에 기여.

  - 대면소통을 증강하기 때문은 아니다. 
  - 미디어이용 자체의 기능. 
  - 미디어이용이 소속연결감 증대. 이를 통한 안녕감 증가. 그러나 부분매개.
  - 사후분석 결과, 관계이용이 정보이용과 오락이용의 안녕감 증대 매개.   
  (관계이용의 안녕감 증가를 매개하는 변수는?)
  
- 미디어이용시간이 많은 집단에서 대면소통의 안녕감/불안녕감 긍정효과 소멸.

  - 미디어이용이 대면소통의 긍정효과를 상쇄하는 작용.
  - 미디어이용 고유의 긍정 및 부정 작용 존재 가능성
